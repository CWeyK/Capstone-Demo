...
new class extends Component {
    ...
    {
        if($this->user->hasRole('Student') || $this->user->hasRole('Lecturer')) {
    ...
    public function groupScheduleByDay(): void
    {
        ...
        if($this->user->hasRole('Student')){
            $groups = $this->user->studentGroups
                ->filter(fn ($group) => $group->time)
                ->sortBy(fn ($group) => Carbon::createFromFormat('H:i', explode('_', $group->time)[1]));
            $this->groupedByDay = [];
        }else if($this->user->hasRole('Lecturer')) {
            $groups = $this->user->classGroups
                ->filter(fn ($group) => $group->time)
                ->sortBy(fn ($group) => Carbon::createFromFormat('H:i', explode('_', $group->time)[1]));
            $this->groupedByDay = [];
        } 
        ...
        if($this->user->hasRole('Student')) {
            $replacements = ClassReplacement::with(['classGroup.subjectClass', 'classGroup.room', 'classGroup.lecturerUser'])
                ...
                ->whereBetween('date', [$weekStart->format('Y-m-d'), $weekEnd->format('Y-m-d')])
                ->whereIn('class_group_id', $this->user->studentGroups->pluck('id'))
                ->get();
        } else if($this->user->hasRole('Lecturer')) {
            $replacements = ClassReplacement::with(['classGroup.subjectClass', 'classGroup.room', 'classGroup.lecturerUser'])
                ->where('status', 'scheduled')
                ->whereBetween('date', [$weekStart->format('Y-m-d'), $weekEnd->format('Y-m-d')])
                ->whereIn('class_group_id', $this->user->classGroups->pluck('id'))
                ->get();
        } 
...
<div>
    <livewire:user.user-details :user="$user" />

    <div class="tab-content">
        @if($user->hasRole('Student') || $user->hasRole('Lecturer'))
        <div class="tab-pane fade show active" id="user_schedule" role="tab-panel" wire:ignore.self>
